    ...
    with mlflow.start_run():
        ...
        with MCAPLogger(mcap_path) as mcap_logger:
            for step in range(max_steps):
                ...
                if step % 100 == 0:
                    print(
                        f"Step {step}: x={current_state.x:.2f}, y={current_state.y:.2f}, v={current_state.velocity:.2f}"
                    )

                # Check if reached end
                dist_to_end = (
                    (current_state.x - track[-1].x) ** 2 + (current_state.y - track[-1].y) ** 2
                ) ** 0.5
        ...
        print("Uploading MCAP file to MLflow...")
