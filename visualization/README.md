...
```bash
uv run python visualization/scripts/plot_logs.py \
  results/log_nn.json \
  -o output.png
```
...
```bash
uv run python visualization/scripts/animate_logs.py \
...
```bash
# MCAPをJSONに変換
uv run python visualization/scripts/mcap_to_log.py \
  simulation.mcap \
  -o log.json

# 変換後、既存のツールで可視化
uv run python visualization/scripts/plot_logs.py log.json -o plot.png
uv run python visualization/scripts/animate_logs.py log.json -o animation.gif
```
...
```python
from visualization import SimulationPlotter, SimulationAnimator
