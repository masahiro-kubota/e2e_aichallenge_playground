...
| テスト対象クラス | 検証項目 | 内容 |
|------------------|----------|------|
| **外部インターフェース** | 概要 | 外部（プランナーやコントローラー）とのやり取りには、標準の `VehicleState` と `AckermannDriveStamped` データクラスが使用されます。内部的な指令管理には `AckermannDrive` が使用されます。 |
...
| **AckermannDrive** | 作成と初期化 | 制御指令（ステアリング、加速度）が正しく設定されるか |
| | Pydanticバリデーション | 不正な値や型の入力に対してエラーを出すか |
...
| (Perception, Planning, Control) | 入出力型 | 各メソッドが規定の型（Observation, AckermannDriveStamped等）を入出力するか |
