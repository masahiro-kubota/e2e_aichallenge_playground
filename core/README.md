...
ノードベースアーキテクチャへの移行に伴い、以下の最適化と機能拡張を計画しています：

- [ ] **パフォーマンス最適化**:
  - ノードグラフのバリデーションやログ収集のオーバーヘッドを評価し、必要に応じて本番環境（推論モード）向けの軽量モードを実装する。
  - データコピークラスの削減（現在は安全性重視でコピーを多用している箇所があるため）。
- [ ] **ノードグラフ可視化の統合**:
  - `visualize_node_graph` をランナーに統合し、実験開始時に自動的にノード構成図（PNG/SVG）を出力するオプションを追加する。
