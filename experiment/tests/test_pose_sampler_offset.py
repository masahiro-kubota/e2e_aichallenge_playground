...
def pose_sampler():
    with (
        patch.object(PoseSampler, "_load_map") as _,
        patch.object(PoseSampler, "_load_track") as _,
...
def test_lateral_offset_distribution_straddle(pose_sampler):
    ...
    ys = []
    for _ in range(50):
        result = pose_sampler.sample_track_pose(lateral_offset_range=lateral_offset_range)
        ys.append(result[1])
