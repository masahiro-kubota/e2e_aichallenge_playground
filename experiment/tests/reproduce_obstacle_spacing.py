...
def test_validate_min_distance_rejection(generator):
    ...
    candidate = {
        "position": {"x": 105.0, "y": 0.0, "yaw": 0.0, "centerline_dist": 105.0},
        "shape": {"width": 2.0, "length": 4.0},
    }
...
def test_validate_min_distance_acceptance(generator):
    ...
    existing = [
        {
            "position": {"x": 100.0, "y": 0.0, "yaw": 0.0, "centerline_dist": 100.0},
            "shape": {"width": 2.0, "length": 4.0},
        }
    ]
    ...
    candidate = {
        "position": {"x": 111.0, "y": 0.0, "yaw": 0.0, "centerline_dist": 111.0},
        "shape": {"width": 2.0, "length": 4.0},
    }
...
def test_validate_same_centerline_index_none(generator):
    ...
    candidate = {
        "position": {"centerline_dist": 100.0, "centerline_index": None, "x": 0, "y": 0, "yaw": 0},
        "shape": {"width": 2, "length": 4},
    }

    # None == None is True, so it should compare distances
    valid = generator._validate_placement(candidate, existing, min_distance=10.0)
    assert not valid
