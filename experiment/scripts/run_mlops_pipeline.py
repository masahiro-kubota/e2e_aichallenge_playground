...
class MLOpsPipeline:
    def __init__(self, args):
        ...
        self.train_collection_dir: Path | None = None
        self.val_collection_dir: Path | None = None
    ...
    def run_aggregation(self):
        ...
        if self.args.dry_run:
            logger.info(
                f"Would aggregate {self.train_collection_dir} and {self.val_collection_dir}"
            )
            return
    ...
    def run_extraction(self):
        ...
        train_input = (
            self.train_collection_dir if self.train_collection_dir else "outputs/TR_DATE/TR_TIME"
        )
        ...
        # Val Extraction
        val_input = (
            self.val_collection_dir if self.val_collection_dir else "outputs/VAL_DATE/VAL_TIME"
        )
        val_cmd = (
            f"uv run experiment-runner "
            f"experiment=extraction "
    ...
    def run_training(self):
        ...
        if not self.args.dry_run:
            ...
            try:
                latest_run_dir = (
                    self._find_latest_multirun()
                )  # Usually single run for training but same logic applies
    ...
    def _save_run_result(self):
        ...
        result_data = {
            ...
            "outputs": {
                "train_collection": str(self.train_collection_dir)
                if self.train_collection_dir
                else None,
            ...
            if d and (d / "collection_summary.json").exists():
                try:
                    with open(d / "collection_summary.json") as f:
                        result_data["intermediate_results"][f"{split}_collection_summary"] = (
                            json.load(f)
                        )
                except Exception as e:
            ...
            if d and (d / "stats.json").exists():
                try:
                    with open(d / "stats.json") as f:
                        result_data["intermediate_results"][f"{split}_extraction_stats"] = (
                            json.load(f)
                        )
                except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--version", type=str, default="v7", help="Dataset/Experiment version (e.g. v7)"
    )
    parser.add_argument(
        "--train-episodes", type=int, default=1000, help="Number of training episodes"
    )
    parser.add_argument(
        "--val-episodes", type=int, default=200, help="Number of validation episodes"
    )

    parser.add_argument("--skip-collection", action="store_true", help="Skip data collection step")
    parser.add_argument(
        "--skip-extraction", action="store_true", help="Skip feature extraction step"
    )
    ...
    parser.add_argument(
        "--model-path",
        type=str,
        help="Path to trained model for evaluation. If not provided, tries to find in latest training output.",
    )
