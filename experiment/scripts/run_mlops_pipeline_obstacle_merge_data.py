...
class MLOpsPipelineMerge:
    def __init__(self, args):
        ...
        self.merge_train_dir = (
            Path(args.merge_train_dir).resolve() if args.merge_train_dir else None
        )
    ...
    def run_merge(self):
        ...
        for split in ["train", "val"]:
            ...
            if merge_dir and merge_dir.exists():
                ...
                if (
                    existing_scans_path.exists()
                    and existing_steers_path.exists()
                    and existing_accelerations_path.exists()
                ):
                    ...
                    logger.warning(
                        f"  Missing scans.npy, steers.npy, or accelerations.npy in {merge_dir}"
                    )
            else:
            ...
            if new_data_dir.exists():
                ...
                if (
                    new_scans_path.exists()
                    and new_steers_path.exists()
                    and new_accelerations_path.exists()
                ):
                    ...
                    logger.warning(
                        f"  Missing scans.npy, steers.npy, or accelerations.npy in {new_data_dir}"
                    )
            else:
            ...
            if existing_scans is not None and new_scans is not None:
                ...
                merged_accelerations = np.concatenate(
                    [existing_accelerations, new_accelerations], axis=0
                )
...
Examples:
  ...
  uv run python experiment/scripts/run_mlops_pipeline_obstacle_merge_data.py \\
    ...
    # Episode counts
    parser.add_argument(
        "--tf-train", type=int, default=0, help="Track Forward Train episodes to collect"
    )
    parser.add_argument(
        "--tf-val", type=int, default=0, help="Track Forward Val episodes to collect"
    )
    parser.add_argument(
        "--rs-train", type=int, default=0, help="Random Start Train episodes to collect"
    )
    parser.add_argument(
        "--rs-val", type=int, default=0, help="Random Start Val episodes to collect"
    )

    # Base seeds (to avoid overlap with existing data)
    parser.add_argument(
        "--tf-train-base-seed", type=int, default=0, help="Base seed for TF train (default: 0)"
    )
    parser.add_argument(
        "--tf-val-base-seed",
        type=int,
        default=100000,
        help="Base seed for TF val (default: 100000)",
    )
    parser.add_argument(
        "--rs-train-base-seed",
        type=int,
        default=200000,
        help="Base seed for RS train (default: 200000)",
    )
    parser.add_argument(
        "--rs-val-base-seed",
        type=int,
        default=300000,
        help="Base seed for RS val (default: 300000)",
    )
    ...
    parser.add_argument(
        "--merge-val-dir", type=str, default=None, help="Path to existing val dataset to merge with"
    )
