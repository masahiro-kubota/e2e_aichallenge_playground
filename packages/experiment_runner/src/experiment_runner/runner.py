...
class ExperimentRunner:
    ...
    def _setup_components(self) -> None:
        ...
        if "track_path" in planning_params:
            ...
            # __file__ is in src/experiment_runner/runner.py
            # Go to packages/components/planning/pure_pursuit/data/tracks/
            components_root = Path(__file__).parent.parent.parent.parent / "components"
            default_track = (
                components_root / "src/components/planning/pure_pursuit/data/tracks"
        ...
        self.planner = self._instantiate_component(
            "components.planning", planning_type, planning_params
        )
        
        # Load track if specified and store for artifact logging
        self.track_path = None
        if track_path is not None:
            from components.planning import load_track_csv
        ...
        self.controller = self._instantiate_component(
            "components.control", control_type, control_params
        )
    ...
    def _run_inference(self) -> None:
        ...
        with mlflow.start_run():
            ...
            if self.config.logging.dashboard.enabled:
                ...
                # Workspace root is 5 levels up from this file
                workspace_root = Path(__file__).parent.parent.parent.parent.parent
