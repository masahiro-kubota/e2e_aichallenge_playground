...
def conv2d(x, weight, bias, stride=(1, 1)):
    ...
    strided_x = as_strided(
        x, shape=(n_x, c_in, h_out, w_out, k_h, k_w), strides=(s0, s1, s2 * s_h, s3 * s_w, s2, s3)
    )

    strided_x_reshaped = strided_x.transpose(0, 2, 3, 1, 4, 5).reshape(
        n_x * h_out * w_out, c_in * k_h * k_w
    )
...
def max_pool2d(x, kernel_size=(2, 2), stride=(2, 2)):
    ...
    strided_x = as_strided(
        x, shape=(n_x, c, h_out, w_out, k_h, k_w), strides=(s0, s1, s2 * s_h, s3 * s_w, s2, s3)
    )
