...
with open(mcap_path, "rb") as f:
    ...
    if steering_messages:
        ...
        for i, msg in enumerate(steering_messages[:10]):
            print(
                f"  {i}: time={msg['time']:.3f}s, steering_tire_angle={msg['steering_tire_angle']:.6f}"
            )

        print("\nLast 10 steering_status messages:")
        for i, msg in enumerate(steering_messages[-10:]):
            print(
                f"  {len(steering_messages) - 10 + i}: time={msg['time']:.3f}s, steering_tire_angle={msg['steering_tire_angle']:.6f}"
            )
