    ...
    for schema, channel, message in reader.iter_messages(
        ...
        if channel.topic == "/vehicle/status/steering_status":
            ...
            steering_data.append(
                {"time": message.log_time / 1e9, "angle": data.get("steering_tire_angle", 0.0)}
            )
...
ax1.plot(
    control_times, control_angles, "b-", label="Control Command (目標)", linewidth=2, alpha=0.7
)
ax1.plot(
    steering_times, steering_angles, "r-", label="Steering Status (実際)", linewidth=2, alpha=0.7
)
...
ax2.plot(
    control_times,
    np.degrees(control_angles),
    "b-",
    label="Control Command (目標)",
    linewidth=2,
    alpha=0.7,
)
ax2.plot(
    steering_times,
    np.degrees(steering_angles),
    "r-",
    label="Steering Status (実際)",
    linewidth=2,
    alpha=0.7,
)
...
print("  Expected (steer_gain): 70%")
print("  Note: Lower ratio is expected due to delay and rate limiting")
