...
def load_and_infer_tinylidarnet(
    ranges: np.ndarray, model_path: Path, max_range: float = 30.0
) -> tuple[float, float]:
    ...
    sys.path.insert(
        0, "/home/masa/python-self-driving-simulator/ad_components/control/tiny_lidar_net/src"
    )
...
def read_first_lidar_ros2(
    mcap_path: Path, target_time_sec: float = 10.0
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare LiDAR scans from ROS2 and Simulator MCAPs"
    )
    parser.add_argument(
        "--time", type=float, default=5.0, help="Target time in seconds from start (default: 5.0)"
    )
    parser.add_argument(
        "--ros2-mcap",
        type=str,
    ...
    print(
        f"  Relative Time: {ros2_info['timestamp_sec']:.3f}s (Absolute: {ros2_info['absolute_timestamp']:.3f}s)"
    )
    print(f"  Frame: {ros2_info['frame_id']}, Points: {ros2_info['num_points']}")
    print(
        f"  Angle: [{ros2_info['angle_min']:.3f}, {ros2_info['angle_max']:.3f}], Increment: {ros2_info['angle_increment']:.6f}"
    )
    ...
    print(
        f"  Angle: [{sim_info['angle_min']:.3f}, {sim_info['angle_max']:.3f}], Increment: {sim_info['angle_increment']:.6f}"
    )
    ...
    print(
        f"    Steer:  {abs(ros2_steer - sim_steer):.4f} rad ({np.degrees(abs(ros2_steer - sim_steer)):.2f}°)"
    )

    # Convert to XY
    ros2_x, ros2_y, ros2_valid = ranges_to_xy(
        ros2_ranges, ros2_info["angle_min"], ros2_info["angle_increment"]
    )
    sim_x, sim_y, sim_valid = ranges_to_xy(
        sim_ranges, sim_info["angle_min"], sim_info["angle_increment"]
    )
    ...
    axes[1].set_title(
        f"Simulator LiDAR @ {sim_info['timestamp_sec']:.2f}s\n({sim_info['frame_id']}, {len(sim_x)} valid points)",
        fontsize=11,
        fontweight="bold",
    )
    ...
    print("\n=== Statistics ===")
    print(
        f"ROS2: Valid={len(ros2_x)}/{len(ros2_ranges)} ({100 * len(ros2_x) / len(ros2_ranges):.1f}%)"
    )
