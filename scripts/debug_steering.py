...
with open(mcap_path, "rb") as f:
    ...
    print("=" * 100)
    print(
        f"{'Index':<6} {'Time':<10} {'Control CMD (rad)':<20} {'Control CMD (deg)':<20} {'Steering Status (rad)':<22} {'Steering Status (deg)':<22} {'Ratio':<10}"
    )
    print("=" * 100)

    # Merge by time
    all_times = sorted(
        set([m["time"] for m in steering_messages] + [m["time"] for m in control_messages])
    )

    for i, t in enumerate(all_times[:30]):
        control_val = next(
            (m["steering_tire_angle"] for m in control_messages if abs(m["time"] - t) < 0.001), None
        )
        steering_val = next(
            (m["steering_tire_angle"] for m in steering_messages if abs(m["time"] - t) < 0.001),
            None,
        )
        ...
        if control_val is not None and steering_val is not None:
            ...
            print(
                f"{i:<6} {t:<10.3f} {control_val:<20.6f} {math.degrees(control_val):<20.3f} {steering_val:<22.6f} {math.degrees(steering_val):<22.3f} {ratio:<10.1f}%"
            )
        elif control_val is not None:
            import math

            print(
                f"{i:<6} {t:<10.3f} {control_val:<20.6f} {math.degrees(control_val):<20.3f} {'N/A':<22} {'N/A':<22} {'N/A':<10}"
            )
        elif steering_val is not None:
            import math

            print(
                f"{i:<6} {t:<10.3f} {'N/A':<20} {'N/A':<20} {steering_val:<22.6f} {math.degrees(steering_val):<22.3f} {'N/A':<10}"
            )
